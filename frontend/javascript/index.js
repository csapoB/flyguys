...
$(async function () {
    ...
    $("#keret").append(await initCheapestFlights((await (await fetch("/api/cheapestflights", {method : "GET", headers: {"Accept-Language": language}})).json()).results, language));
...
async function initCheapestFlights(flights, language) {
    ...
    for (let i = 0; i < flights.one_way.length; i++) {
        ...
        let $price = $("<div>", {
            "class" : "col-12 row border-bottom d-flex justify-content-center pb-2 mb-2",
            "html" : `<div class=\"d-flex justify-content-center align-items-center\"><span class=\"fs-2 fw-bold text-danger\">${flights.one_way[i].BasePrice}</span><span class=\"fs-3\">${getindex.body.cheapest_flights.per_person}</span></div>`
        });
        ...
        "text": getindex.body.cheapest_flights.choose_seats,
        "type": "submit",
        on: {
            "click": function (e) {
                e.preventDefault();
