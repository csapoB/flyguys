...
$(async function () {
    ...
    */
    ...
    $return.prev("label").addClass("text-secondary");
    $.datepicker.setDefaults($.datepicker.regional["hu"]);
    // Egyirányú, vagy oda-vissza gomb
    let $switcher_div_departure_return = $("#switcher_div_departure_return");
    switcher($switcher_div_departure_return);
    $switcher_div_departure_return.on("click", function () {
...
// régi airports_popover_contentGenerator
/*async function airports_popover_contentGenerator(input_field_id, content_div_id, popover_obj) {
    
    let $popover_content = $("<div>", {
        "id" : content_div_id
    });

    let flag_for_airport; // tömb, ami tárolja az előzőleg kiválasztott reptér helyét 

    let airports_data_from_api = await (await fetch("/api/airports", {method : "GET"})).json();

    let airports_of_countries = []; // a reptereket tárolja országonként csoportosítva. Egy eleme felépítése: {ország : "ország neve", repterek : [{város : "város", kód : "kód", elérhető : igen/nem}] }

    
    if (input_field_id == "origin_input") {
        available_airports = (await (await fetch("/api/availabledepartureairports", {method : "GET"})).json()).airportcodes;
    } else { // ha destination_input
        available_airports = (await (await fetch("/api/availablearrivalairports", {method : "GET"})).json()).airportcodes;
    }
    
    airports_of_countries.push({country : airports_data_from_api.results[0].Country, airports: [{city : airports_data_from_api.results[0].City, code : airports_data_from_api.results[0].AirportCode, available : available_airports.includes(airports_data_from_api.results[0].AirportCode)}]});
    for (let i = 1; i < airports_data_from_api.results.length; i++) {

        if (airports_of_countries[airports_of_countries.length-1].country != airports_data_from_api.results[i].Country) {
            airports_of_countries.push({country : airports_data_from_api.results[i].Country, airports: [{city : airports_data_from_api.results[i].City, code : airports_data_from_api.results[i].AirportCode, available : available_airports.includes(airports_data_from_api.results[i].AirportCode)}]});
        } else {
            airports_of_countries[airports_of_countries.length-1].airports.push({city : airports_data_from_api.results[i].City, code : airports_data_from_api.results[i].AirportCode, available : available_airports.includes(airports_data_from_api.results[i].AirportCode)});
            
        }
    }

    for (let i = 0; i < airports_of_countries.length; i++) {

        let $airports_frame = [];

        if (i == airports_of_countries.length-1) {
        ...
        let $span = $("<span>", {
            "class" : "text-danger",
            "text" : airports_of_countries[i].country
        });
        $span.css({userSelect : "none"}); // nem lehet kimásolni a szöveget

        $airports_frame.append($span);
        
        for (let j = 0; j < airports_of_countries[i].airports.length; j++) {
            
            let $airport_div = $("<div>", {
                "class" : "pt-2"
            });
            $airport_div.data("code", airports_of_countries[i].airports[j].code);

            let $airport_span = $("<span>", {
                "class" : "fs-6 p-2",
                "text" : `${airports_of_countries[i].airports[j].city} (${airports_of_countries[i].airports[j].code})`
            });
            
             // ha nem indul járat (origin_inputnál oda, destination_:inputnál vissza), akkor ne lehessen kiválasztani
            $airport_span.css({userSelect : "none"}); // nem lehet kimásolni a szöveget

            $airport_div.append($airport_span);

            if (airports_of_countries[i].airports[j].available){
                ...
                $airport_div.on("click", function (){
                    ...
                    if (flag_for_airport != undefined ) {
                        ...
                        if (($thisDiv.parent().index() != flag_for_airport[0] || $thisDiv.index() != flag_for_airport[1])) {

                            $thisDiv.parent().parent().children().eq(flag_for_airport[0]).children().eq(flag_for_airport[1]).children().first().removeClass("rounded-pill bg-danger text-light");

                            flag_for_airport = [$thisDiv.parent().index(), $thisDiv.index()];

                            $thisDiv.children().first().addClass("rounded-pill bg-danger text-light");

                            $("#" + input_field_id).val($thisDiv.children().first().text()); // beilleszti a kiválasztott repülőtér szövegét az inputba

                            popover_obj.hide();

                            let $input_field = $("#" + input_field_id);
                            $input_field.data("code_of_selected_airport", $thisDiv.data("code"));
                            $input_field.trigger("change");
                ...
                $airport_span.addClass("text-secondary");
...
function inputSwitcher($input) {
    ...
    if ($input.prop("disabled")) {
        ...
        $input.prev("label").removeClass("text-secondary");

    } else {

        $input.prop("disabled", true);
        $input.prev("label").addClass("text-secondary");
